...
**Lifecycle transitions (from ddd.md §7):**
...
- `EVT-EventCancelled` — raised when an event is Cancelled. This is an **integration event** that will fan out to Sales, Payments, Ticketing, and Notifications (per ddd.md §6) when those consumers are built.
...
- The `VO-EventStatus` value object already supports `Closed` and `Cancelled` states (as defined in ddd.md).
- The Event aggregate already has `Close` and `Cancel` behaviors defined (as listed in ddd.md §4 BC-2).
