...
**Storage principle (prd.md QG-6, technical.md §5):** Avatar image bytes are stored in MinIO (object storage). The User aggregate persists only the object key/URL reference (`VO-AvatarImageRef`), never the binary content.
