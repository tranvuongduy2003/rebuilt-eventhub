...
**Value object: `VO-SalesWindow`** (defined in `ddd.md`)
...
**Invariant INV-14** (`ddd.md`): A reservation can only succeed when the event is `Published`, the ticket type's sales window is currently open (or absent), and the event is not `Closed` or `Cancelled`. This invariant is the enforcement point for the sales window.
